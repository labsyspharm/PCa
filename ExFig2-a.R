...
# Calculate % B cells and % CD8 T cells in tumor for high group
h_percent_B <- numeric(length(high))
h_percent_CD8 <- numeric(length(high))
c <- 1
for (i in high) {
  ...
  if (nrow(tumor) > 0) {  # Guard against empty tumor subsets
    ...
    h_percent_CD8[c] <- sum(tumor$phen_vec == "CD8 T cells") / nrow(tumor) * 100
  } else {
    h_percent_B[c] <- NA
    h_percent_CD8[c] <- NA
  }
...
# Calculate % B cells and % CD8 T cells in tumor for low group
l_percent_B <- numeric(length(low))
l_percent_CD8 <- numeric(length(low))
c <- 1
for (i in low) {
  l <- ldata[[i]]
  tumor <- subset(l, bitAnd(cflag, 8) == 8)
  if (nrow(tumor) > 0) {
    ...
    l_percent_CD8[c] <- sum(tumor$phen_vec == "CD8 T cells") / nrow(tumor) * 100
  } else {
    l_percent_B[c] <- NA
    l_percent_CD8[c] <- NA
  }
...
data <- data.frame(
  ...
  percent_CD8 = c(h_percent_CD8, l_percent_CD8),
...
fit_HGG <- lm(percent_CD8 ~ percent_B, data = data[data$Group == "HGG", ])
fit_LGG <- lm(percent_CD8 ~ percent_B, data = data[data$Group == "LGG", ])
...
ggplot(data, aes(x = percent_B, y = percent_CD8, color = Group)) +
  ...
  labs(title = "B and CD8 T cells regression",
       x = "% B cells on the tumor",
       y = "% CD8 T cells on the tumor") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"),
        legend.position = "bottom",
        legend.title = element_blank()) +
  annotate("text", x = max(data$percent_B) * 0.7, y = max(data$percent_CD8) * 0.9,
           label = paste("HGG P =", formatC(p_value_HGG, digits = 4, format = "f")),
           color = "#7570b3") +
  annotate("text", x = max(data$percent_B) * 0.7, y = max(data$percent_CD8) * 0.8,
           label = paste("LGG P =", formatC(p_value_LGG, digits = 4, format = "f")),
           color = "#1b9e77") +
  annotate("text", x = max(data$percent_B) * 0.9, y = min(data$percent_CD8) + 1,
...
# Calculate % B cells and % CD4 T cells in tumor for high group
h_percent_B <- numeric(length(high))
h_percent_CD4 <- numeric(length(high))
c <- 1
for (i in high) {
  l <- ldata[[i]]  # Use direct name indexing as in file
  tumor <- subset(l, bitAnd(cflag, 8) == 8)
  if (nrow(tumor) > 0) {  # Guard against empty tumor subsets
    h_percent_B[c] <- sum(tumor$coarse_phen_vec == "B cells") / nrow(tumor) * 100
    h_percent_CD4[c] <- sum(tumor$phen_vec == "CD4 T cells") / nrow(tumor) * 100
  } else {
    h_percent_B[c] <- NA
    h_percent_CD4[c] <- NA
  }
...
# Calculate % B cells and % CD4 T cells in tumor for low group
l_percent_B <- numeric(length(low))
l_percent_CD4 <- numeric(length(low))
c <- 1
for (i in low) {
  l <- ldata[[i]]
  tumor <- subset(l, bitAnd(cflag, 8) == 8)
  if (nrow(tumor) > 0) {
    l_percent_B[c] <- sum(tumor$coarse_phen_vec == "B cells") / nrow(tumor) * 100
    l_percent_CD4[c] <- sum(tumor$phen_vec == "CD4 T cells") / nrow(tumor) * 100
  } else {
    l_percent_B[c] <- NA
    l_percent_CD4[c] <- NA
  }
...
data <- data.frame(
  percent_B = c(h_percent_B, l_percent_B),
  percent_CD4 = c(h_percent_CD4, l_percent_CD4),
  Group = rep(c("HGG", "LGG"), c(length(high), length(low)))
)
data <- na.omit(data)  # Remove specimens with no tumor data

# Perform linear regressions for each group (exact pattern from fit_high/fit_low in file)
fit_HGG <- lm(percent_CD4 ~ percent_B, data = data[data$Group == "HGG", ])
fit_LGG <- lm(percent_CD4 ~ percent_B, data = data[data$Group == "LGG", ])
...
# Plot with matching aesthetics: scatter, regression lines, 95% CI shading, colors, annotations
# (Pattern from violin plot and end-of-file boxplots, with geom_smooth for lm + CI)
ggplot(data, aes(x = percent_B, y = percent_CD4, color = Group)) +
  geom_point(size = 2, alpha = 0.8) +  # Points for specimens
  geom_smooth(method = "lm", fill = "grey70", alpha = 0.3, level = 0.95) +  # Regression + gray 95% CI
  scale_color_manual(values = c("HGG" = "#7570b3", "LGG" = "#1b9e77"),
                     labels = c("HGG specimen", "LGG specimen")) +
  labs(title = "B and CD4 T cells regression",
       x = "% B cells on the tumor",
       y = "% CD4 T cells on the tumor") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"),
        legend.position = "bottom",
        legend.title = element_blank()) +
  annotate("text", x = max(data$percent_B) * 0.7, y = max(data$percent_CD4) * 0.9,
           label = paste("HGG P =", formatC(p_value_HGG, digits = 4, format = "f")),
           color = "#7570b3") +
  annotate("text", x = max(data$percent_B) * 0.7, y = max(data$percent_CD4) * 0.8,
           label = paste("LGG P =", formatC(p_value_LGG, digits = 4, format = "f")),
           color = "#1b9e77") +
  annotate("text", x = max(data$percent_B) * 0.9, y = min(data$percent_CD4) + 1,
           label = "95% CI", color = "grey50", angle = 30, size = 3.5)
