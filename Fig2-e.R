...
for (r in radii) {
  ...
  if (r == 1.1){ symbols(0, 0, circles = r, inches = FALSE, add = TRUE, fg = "lightgray", lwd = 0.7)}
...
for (i in 1:length(values)) {
  ...
  if (names(ldata)[i] %in% names(mat)) {
    ...
    text(x_pos_inner3, y_pos_inner3, labels = (length(unique(ldata[[i]]$tls_id))-1) - (dim(mat[[which(names(mat)==names(ldata)[i])]])[2]), cex = 1.3)
    err<- runif(1, -0.1, 0.1)
