...
## Clean data (finite Var1/log_Var4 and non-missing Group)
dff_clean <- dff %>%
  filter(is.finite(Var1), is.finite(log_Var4), !is.na(Group), Var1 < 90) #removing 2 huge outliers

## Split groups (for modeling)
dff_high <- filter(dff_clean, Group == "High")
dff_low  <- filter(dff_clean,  Group == "Low")

## Fit simple linear models (finite already ensured)
fitBk_high <- lm(log_Var4 ~ Var1, data = dff_high)
fitBk_low  <- lm(log_Var4 ~ Var1, data = dff_low)
...
p_value_high <- f_pval(fitBk_high)
p_value_low  <- f_pval(fitBk_low)

## R-squared
r2_high <- summary(fitBk_high)$r.squared
r2_low  <- summary(fitBk_low)$r.squared

## Annotation coordinates
x_annot  <- min(dff_clean$Var1, na.rm = TRUE) + 2
y_max    <- max(dff_clean$log_Var4, na.rm = TRUE)
y_annot1 <- y_max - 0.5
y_annot2 <- y_max - 1.0

## Plotting data (remove y = 0 only for display)
df_all_nz  <- filter(dff_clean, log_Var4 != 0)
df_high_nz <- filter(dff_high,  log_Var4 != 0)
df_low_nz  <- filter(dff_low,   log_Var4 != 0)

## Plot
ggplot(df_all_nz, aes(x = Var1, y = log_Var4)) +
  geom_point(aes(color = Group), size = 3, na.rm = TRUE) +
  geom_smooth(data = df_high_nz, aes(x = Var1, y = log_Var4),
              method = "lm", color = "#7570b3", se = TRUE, na.rm = TRUE) +
  geom_smooth(data = df_low_nz,  aes(x = Var1, y = log_Var4),
              method = "lm", color = "#1b9e77", se = TRUE, na.rm = TRUE) +
  scale_color_manual(values = c(High = "#7570b3", Low = "#1b9e77")) +
  labs(
    x = "ICAT",
    y = "log(CD20+Ki67+ prop)",
    title = "CD20+Ki67+ and ICAT regression"
  ) +
