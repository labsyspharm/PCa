...
##this is loading the needed functions and build the data needed for the panesl of the Figure 3 and Extended Data Figure 3
...
ICA <- read_csv("/Users/aliamiryousefi/HMS Dropbox/Ali Amiryousefi/Primary_prostate_CyCIF_TME_manuscript_2023/Analysis_Ali/GU-material2/ICA_titles_ranked.csv") %>%
  filter(flag %in% c("P","T","N")) %>%
  # put T first, then P, then N
  mutate(flag = factor(flag, levels = c("T","P","N")))
...
df$second_part <- sub(".*@", "", df[,1])
colnames(df)[2] <- "ICAT"

# Step 1: extract the matching key from ICA$title
ICA$match_key <- sub(".* / ", "", ICA$title)  # extract "4@LSP12601" part

# Step 2: create a named lookup vector from ICA
flag_lookup <- setNames(as.character(ICA$flag), ICA$match_key)

# Step 3: map df$TLS to flag values using the lookup vector
df$flag <- flag_lookup[df$TLS]
