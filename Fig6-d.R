...
for(i in seq_len(nrow(Gr_Tu_ind))) {
  ...
  ctl_mask   <- lp$phen_vec=="CD8 T cells" & lp$phen_vec=="CD8 T cells" & log(lp$GranzymeB)>gate 
...
for(i in seq_len(nrow(Gr_Tu_ind))[c(-16,-22, -23)]) {#samples without tumor and poor GZB 
  samp  <- Gr_Tu_ind$sample[i]
  gate  <- Gr_Tu_ind$gate [i]
  lp    <- ldata[[samp]]
...
save(GranzymeTumor, file="GranzymeTumor.RData")


###plot the contact group.

# Re-filter NAs & outliers
