...
for (i in seq_along(high)) {
  ...
  df <- ldata[[sample]]
  numerator <- sum(log(df$GranzymeB) > gate & df$phen_vec == "CD8 T cells" & bitAnd(df$cflag, 8) == 8)
...
test_result <- wilcox.test(Fraction ~ Group, data = df, alternative = "greater")
...
ggplot(df, aes(x = Group, y = Fraction, fill = Group)) +
  ...
  labs(
    title = paste0("GZB+CD8+ fraction of T cells (Mann-Whitney, p = ", pval, ")"),
    x = "Gleason status",
    y = "Cells in tumor"
  ) +
